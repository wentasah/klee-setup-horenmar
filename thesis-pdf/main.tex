...
\cite[symbolic-execution-article].

The contents of this section are heavily based on the KLEE paper%
\cite[klee-paper] and \cite[s2e-paper]
...
Cloud9 is a symbolic executor that can scale over clusters of machines, and
...
S2E is a platform for analysing large scale programs In-Vivo. It accomplishes
...
SymDrive\urlnote{http://research.cs.wisc.edu/sonar/projects/symdrive/} is tool
for testing Linux and FreeBSD drivers without hardware.\cite[symdrive-paper]
It uses static-analysis to automatically find driver entry points and loops,
which often allows it to test a driver without requiring any modifications
by its author and if it cannot create modified driver automatically, it alerts
the developer as to what changes are required.\cite[symdrive-paper]

Kite is a KLEE based that prunes its search space whenever a path is proven
