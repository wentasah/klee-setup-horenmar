...
\rfc{Mention soundness and/or lack thereof depending on the tools?}
...
\cite[linux-kernel-vuln].

\rfc{Introduce possible bugs and give some background? Overflow might be UB,
dereferencing null pointers, overshifts, going outside of memory...}
...
\begitems
* Assertion violation
* Access outside of allocated memory (including null pointer dereference)
* Division by zero
* Overshift (shifting an integer by more than its size)
\enditems
...
this I ended up having to change significant parts of the code, compromising
completeness of testing coverage to be able to perform logic analysis.

\rfc{specify changes}
All my modifications are publicly available as a fork to the original project.%
