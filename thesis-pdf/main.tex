...
already existing deployments of CAN.
...
Ravitch. WLLVM works by using a compiler capable of emitting both normal object
...
binary~\cite[wllvm-working]. KLEE can then run the bitcode program.
...
MaCAN network and attempts to adjust local time to keep it in sync. As can be
seen in Figure~\ref[over-code] it calculates time in microseconds and places
the result into 64 bit unsigned integer which is large enough, but the
arithmetic itself is done with 32 bits of precision and leads to erroneous
results.
