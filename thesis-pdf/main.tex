...
in our testing, we are only interested in messages marked with ">>". There
are placeholder number of messages that we need to simulate a node startup,
and all messages afterwards are a payload.

We have sent a total of 30 messages, which means that our node has received
enough messages to perform initial initialization and placeholder number
messages extra. This means that our testing of the node could exercise all
paths through the MaCAN library, except the cryptographic ones.
