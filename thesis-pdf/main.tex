...
% to comment it out preventively. [3-4, 7] is ugly as fuck.
...
and whether using KLEE is viable going forward. I conclude that KLEE can be
a valuable tool to use for verification of real-time safety-critical software,
but the software has to be designed with having KLEE in mind.}
...
\thanks{I would like to thank my supervisor Michal Sojka for his patience,
...
Safety-related software is software whose failure might result in injuries,
and property damage. More people rely on it every day. It is characterized
by being thoroughly tested and reliable, but as the Internet of Things is
becoming more and more pervasive, safety cannot be achieved without considering
security, as was shown by the recent example of being able to locate and unlock Tesla
Model S electric car by hacking a trivially brute-forceable security
code~\cite[tesla-security].

Because security used to be thought of as completely orthogonal to
safety-related software, traditionally used tools for verification of
safety-related software are ill-prepared to expose security vulnerabilities,
we decided to try a verification method oriented towards finding bugs
in general and evaluate its fitness for testing safety-related software in
particular.

The verification method we decided to use is symbolic execution, using
KLEE~\cite[klee-paper] as our executor. Because symbolic execution is generally
sound and its main drawback, exponential runtime complexity in number of
executed branches, is  mitigated by the fact that safety-related software
contains relatively low number of paths through its code, it seemed as a good
fit for our needs. And while no tool can prevent security
vulnerabilities caused by naive authentication scheme, automatic testing
can find security vulnerabilities caused by {\em implementation} bugs which
can allow an attacker to bypass security measures that are in place.

This thesis investigates KLEE's fitness for purpose of testing safety-related
software, documents its results on two selected pieces of safety-related
software, eMotor and MaCAN library, and evaluates whether symbolic execution
tools should be used for testing safety-related software in the future.
...
safety-related software, symbolic execution and existing tools using it,
the tool I am using in this thesis and the two pieces of safety-related
software we will use to evaluate KLEE's fitness for purpose. Third chapter,
Toolchain and case study preparation, covers work done on KLEE, eMotor software
and the MaCAN library in the course of writing my thesis. Fourth chapter,
Evaluation, covers our findings, such as number of bugs found using KLEE, their
severity, time spent on finding them and whether KLEE will be useful tool for
testing real-time safety-related software going forward. The final chapter,
Conclusion, recapitulates our findings and recommendations.
...
are required to have their responses fully defined for all possible inputs.
This means that software contained
...
unsolvable. Consider comparing cryptographic hash of a symbolic value against
predetermined result. To be able to give proceed, a symbolic tool would have
to reverse a cryptographic hash which is generally regarded as impossible
without brute-force testing the input space, and
in specific cases might be impossible completely. Other problematic constructs
...
The eMotor driver is Infineon Technologies' proprietary electric motor control
software module for AUTomotive Open System ARchitecture
(AUTOSAR\urlnote{http://www.autosar.org/}), standardized software architecture
for automotive domain. It has an interrupt driven design (the entire control
algorithm is executed inside the interrupt handler) and is designed to run on
32-bit Tricore TC1798 microcontroller. It requires proprietary IDE, compiler,
assembler and linker to be compiled.
...
Because KLEE's overflow checking relies on Clang's Undefined Behavior Sanitizer,
...
Because eMotor's hardware-dependent code is weakly abstracted, modifying
eMotor to support symbolic execution has shown itself to be hard and very time
consuming. Because of time issues and the fact that the project for which
we worked on eMotor has ended, I talked to my supervisor and we decided
to give up on testing eMotor.
...
its search strategy. Total numbers of bugs found during both runs are in
Table~\ref[klee-results].

\midinsert \clabel[klee-results]{Overview of bugs found}
\ctable{lr}{
...
\caption/t Overview of bugs found.
\endinsert
...
\midinsert \clabel[overflow-results]{Overview of overflows found}
\ctable{lr}{
...
\caption/t Overview of overflows found.
\endinsert
...
"t->expire_us = now + t->repeat_us" whose right-hand side can overflow and the
...
be difficult if its hardware-dependent code is weakly abstracted. This limits
the kinds of software that can be tested using KLEE, to those with well
abstracted inputs, which usually means software that was written to be
cross-platform.
...
test program for the MaCAN library. Due to an error, made while configuring
...
to test a program. The MaCAN test setup described previously has been running
on modestly fast CPU\fnote{2GHz AMD Opteron 6128} for 5 days without finishing.
This is caused by
...
DFS search strategy, where "Instrs" is number of executed LLVM IR instructions,
...
percentage of time spent in the SAT solver. As the figure shows, KLEE has
achieved approx. 77\% instruction coverage and approx. 60\% branch coverage.
...
Because most safety-critical programs are hardware dependent and their execution is
expected to never terminate, the software given to KLEE has to be modified and these
modifications can potentially obscure existing bugs.
Assuming no bugs were obscured by testing-enabling modifications, KLEE's analysis
...
for software that resembles eMotor, having weakly abstracted hardware-dependent code,
...
For software that does not use all of KLEE's capabilities, e.g. it does not use
assertions, but rather uses it to find undefined behaviour invoking code, there
also exist tools that sacrifice soundness of analysis for speed. An example of
such tool is the STACK~\cite[MIT-ub].
...
The CD enclosed with this thesis contains digital copy of thesis itself in the
"thesis-pdf" folder, all code needed to repeat my tests, together with patches
I made while working my thesis, is in the "thesis-setup" folder and all raw
results from KLEE runs I used to make the final report in the "thesis-data"
folder.

The "thesis-pdf" folder also contains the \TeX document needed to recreate this
thesis and cited papers (for obvious reasons, resources cited from the web are
not included).

The "thesis-setup" folder contains: tarballs of the LLVM-3.3 toolchain together
Clang and compiler-rt, folder "thesis-setup-stable" containing all other
dependencies, that is KLEE, STP, WLLVM, MaCAN, freshly checked out at the git
version I worked with. eMotor is not there because of its proprietary nature
and an NDA I had to sign to work with it. There are also "HOW_TO_INSTALL" and
"HOW_TO_USE" files, which are exactly what they say.

The "thesis-data" folder contains a "readme.txt" file that, together with
Appendix D, explains the "thesis-data" folder organization. Each run's folder
has a "klee-last" folder, containing all output from KLEE created during
that run.
...
This appendix contains list of all flags given to KLEE when testing the MaCAN
library, an overview of results and statistics of all runs.

I used three independent instances of KLEE to test the MaCAN library. I shall
refer to them as "covnew", "dfs10" and "dfs15". Abbreviated contents of their
respective "info" files are in Figure~\ref[klee-cmd-covnew],
Figure~\ref[klee-cmd-dfs10] and Figure~\ref[klee-cmd-dfs15] respectively.

\midinsert \clabel[klee-cmd-covnew]{Contents of {\tt info} file for {\tt covnew}}
\begtt
...
\endtt
\caption/f Contents of {\tt info} file for {\tt covnew}.
\endinsert

\midinsert \clabel[klee-cmd-dfs10]{Contents of {\tt info} file for {\tt dfs10}}
\begtt
klee --optimize --max-memory=0 --search=dfs --check-overshift \
--check-div-zero
Started: 2015-05-14 15:49:56
\endtt
\caption/f Contents of {\tt info} file for {\tt dfs10}.
\endinsert

\midinsert \clabel[klee-cmd-dfs15]{Contents of {\tt info} file for {\tt dfs15}}
\begtt
...
\endtt
\caption/f Contents of {\tt info} file for {\tt dfs15}.
\endinsert

The difference between "dfs" and "covnew" is that KLEE is allowed to use its
own heuristics for guiding the search in the "covnew" invocation. The
difference between "dfs10" and "dfs15" is whether the MaCAN hardware
implementation for KLEE serves 10 received messages in row, or 15.

\midinsert \clabel[klee-runs-bugs]{Overview of KLEE runs results}
\ctable{lr}{
\hfil Type & Number of potential problems found \crl \tskip4pt
    10dfs & 8 \cr
    15dfs & 5 \cr
    covnew & 3 \cr
}
\caption/t Overview of KLEE runs results.
\endinsert

Table~\ref[klee-runs-bugs] shows an overview of KLEE runs and number of
potential bugs they reported, and Figure~\ref[klee-runs-stats] shows their
runtime statistics.

\midinsert \clabel[klee-runs-stats]{Runtime statistics across used tests}
\begtt
----------------------------------------------------------------------
|  Path | Instrs |  Time(s)|  ICov(%)|  BCov(%)|  ICount| TSolver(%) |
----------------------------------------------------------------------
...
----------------------------------------------------------------------
\endtt
\caption/f Runtime statistics across used tests.
\endinsert

Interestingly, even though "covnew" and "15dfs" have been started only 6
minutes apart and stopped inside couple of minutes of each other as well,
for the "covnew" run KLEE reports that it has ran for only about tenth of the
time "15dfs" has ran for. The exact reason for this is unknown, but I speculate
that it is because the "covnew"'s memory consumption quickly ballooned up to
approx. 3.2 GBs and then it hit a KLEE bug with memory consumption. This is
supported by two facts. First, an empirical observation: When trying to run
KLEE with given command line options again, it quickly hit 3.2 GBs of memory
consumption and then its memory consumption doesn't change for hours. Secondly,
there are currently known bugs in KLEE's memory management and it might be
related. This is supported by fact that while using the DFS search strategy,
KLEE's memory consumption is a lot smaller and KLEE executes normally.

Taking these facts into account, together with overview in
Table~\ref[klee-runs-bugs] and test results from Chapter 4,
I have to recommend using KLEE command line invocation from
Figure~\ref[klee-cmd].

\midinsert \clabel[klee-cmd]{KLEE command line invocation}
\begtt
klee --only-output-states-covering-new --optimize --search=dfs \
     --max-memory=0 --check-div-zero --check-overshift <bitcode file>
\endtt
\caption/f KLEE command line invocation.
\endinsert
