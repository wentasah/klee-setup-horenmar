...
authentication scheme, automatic testing can find security implementation
bugs which could potentially allow an attack to bypass security measures that
are in place. Thus, in this thesis I attempt to determine whether a general
purpose symbolic execution tool, KLEE, can help find these bugs in existing
...
assembly~\cite[s2e-paper], LLVM's IR (from C/C++)~\cite[klee-paper, c9-paper]
...
driver entry points and loops, which often allows it to test a driver without
requiring any modifications by its author and if it cannot create modified
driver automatically, it alerts the developer as to what changes are 
required~\cite[symdrive-paper].%
...
KLEE takes a file of LLVM IR as its input and works directly on the LLVM IR. It
is capable of interpreting a large subset of the language, that does not
include symbolic floating point, threads, "longjmp", embedded assembly code and
some intrinsics. All memory objects also have to have concrete size.

At the core of KLEE testing is an interpreter loop, selecting a currently open
state and executing an LLVM instruction using selected state's context. State's
context is comprised of corresponding registers, stack and heap objects. If the
instruction does not branch, it is executed as normal, but if the executed
instruction is a conditional branch instruction, KLEE uses a SAT solver to
determine whether the condition is provably true or false given current state
and constraints created along the path taken to reach it, in which case KLEE
takes the appropriate path. If the conditional branch cannot be proved to be
either, KLEE creates two new states with added constraints from the branch.
These two states are then added to priority queue of all open states.
...
\rfc{Should I also describe internal optimizations?}
...
In this chapter I intend to explain basic usage of KLEE. I will be working with
two toy single file programs, "sum.c" and "memcpyassert.c", whose listings are
in figure~\ref[sum-listing] and figure~\ref[memcpyassert-listing] respectively.
It assumes already working KLEE and Clang-3.3.
...
the source file as seen in figure~\ref[compilation-flags]. The result of this
is a ".o" file containing LLVM bitcode. This can be now run by KLEE as seen in
figure~\ref[how-to-run-klee].

\midinsert \clabel[compilation-flags]{compilation flags}
\begtt
clang -emit-llvm -c -g -fsanitize=integer <name>.c
\endtt
\caption/f compilation flags
\endinsert
...
integer overflow, which is UB and for "memcpyassert.o" it is a possible
assertion violation.
...
module itself. It has interrupt driven design (the entire control algorithm is
...
architectures~\cite[MaCAN-paper]. It was designed to be cross-platform by only
...
these two libraries using KLEE. It will also cover other necessary preparations
and problems I had to face along the way, touching upon some ``personal''
experience gained during this thesis.
\rfc{reword preparations}
...
When I started working with the KLEE tool, it had LLVM-2.9 and LLVM-GCC as its
dependencies, which at the time were about 3 years outdated. Since then there
was significant work done to allow KLEE to work with newer version of the LLVM
...
combined with time issues and the fact the project for which we worked on
eMotor has ended, led us to give up on testing eMotor.
...
\ctable{lr}{
\hfil type of overflow & number found \crl \tskip4pt
    benign & placeholder \cr
    causing bugs & placeholder \cr
    causing undefined behaviour & placeholder \cr
}
...
MaCAN at an exhibition and this was the implemented fix:
\begtt
uint32_t time_ts;
uint64_t time_ts_us;
...
