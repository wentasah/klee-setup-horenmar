...
By itself, KLEE can find and show paths that lead to 

    - Null pointer dereference
    - Access outside of allocated memory
    - Unsatisfyable conditions \rfc{???}
    - Assertion violation

\secc KLEE integer overflow checking

When I started to work on using KLEE for testing real-time safety-critical
software, it was unable to detect integer overflow.

In a case of parallel development, both I and \rfc{ToDO? Reformulate}
added this capability to KLEE, and \rfc{Todo!}'s patches have been since merged
into mainline KLEE.

KLEE is currently relying on Clang's support for UndefinedBehaviorSanitizer
\rfc{check name} which in turn is based on Regehr et al's work on integer
overflow checking. \cite[ioc-paper] KLEE implements overflow checking
