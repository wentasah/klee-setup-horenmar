...
which in turn is based on work by Regehr et al. on integer overflow checking
\cite[ioc-paper]. KLEE implements overflow checking by performing arithmetic
operations as-if done on double sized types and then checking upper half of the
result. If it is not empty, the operation must have overflown.
