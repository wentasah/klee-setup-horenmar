...
ohranièené pomocí "\csname...\endcsname".
...
nebo stránku v místì pou¾ití. Zlom se uskuteèní, chybí-li do konce
...
"\replacestringsA". To je udìláno pomocí triku s "\edef\tmpb" a se dvìma "\expandafter". 
...
v "\tmpb" ulo¾en tøeba text "ahaXXuffXXkonec". Makro "\replacestringsA" je v
takovém pøípadì definováno jako:
...
\addto\tmpb{konec?}, tj. \tmpb obsahuje "ahaYYuffYYkonec?" a rekurze konèí.
\endtt
%
...
verzi \csplain{}u na terminálu a potøebná makra si definuje. Je to kopie
kódu ze souboru "csfontsm.tex" z balíèku \csplain{}.
...
"\input \jobname.ref". V takovém pøípadì po naètení REF souboru jej otevøe 
k~zápisu a pøipraví "\wref" do stavu, kdy toto
...
je¹tì není zalo¾en, makro jej zalo¾í, pøedefinuje "\wref" a vlo¾í první
øádek do souboru. Tím je zaruèeno, ¾e pøi pøí¹tím \TeX{}ování dokumentu 
je soubor neprázdný, tak¾e jej OPmac rovnou pøeète a znovu zalo¾í na zaèátku
své èinnosti. Nakonec se "\openref" zasebevra¾dí, aby se nemuselo pøi
opakovaném volání obtì¾ovat vykonávat nìjakou práci. Práce u¾ je toti¾
hotova.
...
tedy jako odstavec s~pru¾nými "\leftskip" a "\rightskip". Pøíkaz "\unskip"
tìsnì za parametrem "#1" odstraní mezeru z konce øádku, která tam obvykle
je. Teprve poté je nadpis øádnì centrován.
...
vyu¾ito makro \db wcontents, které provede 
\bgroup\def\begtthook{\langleactive}
\begtt
...
mají parametry "{<èíslo>}{<text>}{<strana>}" a jsou definovány následovnì:

\inext{Xchap}{\empty}{+-}

Makro \db tocline "{<odsazení>}{<font>}{<èíslo>}{<text>}{<strana>}" vytvoøí
øádek obsahu. Øádek tiskneme jako odstavec, proto¾e "<text>" mù¾e být tøeba
del¹í. Registr "\leftskip" nastavíme jako souèin "<odsazení>" krát "\iindent".
...
Makro \db draft vlo¾í do "\headline" box nulové vý¹ky a ¹íøky "\draftbox",
...
V makru \db outlinesA "{<odsazení>}{<font>}{<èíslo>}{<text>}{<strana>}"
...
V makru \db outlinesB "{<odsazení>}{<font>}{<èíslo>}{<text>}{<strana>}"
vkládáme jednotlivou polo¾ku obsahu do zálo¾ek pomocí pdf\TeX{}ového
primitivu\hfil\break
"\pdfoutline goto name{<lejblík>} count<potomci> {<text>}".
Èíslo "<potomci>" je opatøeno znaménkem mínus právì tehdy, kdy¾ chceme, aby
polo¾ka ve výchozím stavu nezobrazovala své potomky, ale jen trojúhelníèek.
Potomci se zobrazí a¾ po kliknutí na trojúhelníèek. V makru \db outlinelevel
máme makrem "\outlines" pøipravenu úroveò rozevøení, kterou si u¾ivatel
pøeje. Nejprve pøiètením "\count<odsazení>" dostaneme øadu
...
makro vytiskne pøeètený øádek do dokumentu. Pøed øádkem mù¾e být v "\llap" 
vyti¹tìno èíslo øádku. Zále¾í na hodnotì "\ttline". Je to naprogramováno v
souladu s u¾ivatelskou dokumentací.
...
OPmac pøeddefinuje tøi "<znaky>" pro "<deklaraci>", sice "<znaky>" "c", "l", "r"
...
definováno dal¹í písmeno "P" pro "<deklaraci>". Písmeno "P" vymezí 
tabulkovou polo¾ku, je¾ má stanovenou ¹íøku a del¹í text se láme 
do více øádkù. Je mo¾né si vyzkou¹et tøeba tento kód:
...
\Pwidth=3cm \table{|c|P|}{\crl \tskip3pt
   aaa & Tady je del¹í textík, který se nevejde na øádek. \crl \tskip3pt
   bb  & A tady je taky je nìco del¹ího. \crl}
\endtt
...
\db setpagedimensA "(<¹íøka>,<vý¹ka>)<jednotka>&", k~to\-mu musí 
dopøedu vyexpandovat obsah makra "\pgs:<formát>". To provedeme pomocí tøí
"\expandafter".
