...
\glos {UB} {Undefined Behaviour --- A behaviour for which the C standard gives
            no guarantees}
\glos {CVE} {Common Vulnerabilities and Exposures --- A reference of publicly
             known information security vulnerabilities}
\glos {SAT} {Abbreviation of Boolean Satisfiability Problem --- Determining
             whether given boolean formula can be satisfied}
\glos {IDDFS}{Iterative Deepening Depth First Search --- A Depth First Search
              with increasing depth limit}
\glos {Clang} {LLVM's C, C++ and Objective-C frontend}
\glos {compiler-rt} {LLVM's library for low-level, target-specific runtime
                     components}
\glos {ubsan} {Undefined Behaviour Sanitizer --- A runtime instrumentation library
